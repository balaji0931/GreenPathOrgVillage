...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Red flag management routes
  app.get('/api/red-flags/village/:villageId', requireAuth, requireRole(['manager']), async (req, res) => {
    try {
      const { villageId } = req.params;
      
      if (req.session.villageId !== villageId) {
        return res.status(403).json({ message: "Access denied to this village" });
      }
  ...
  app.get('/api/red-flags/household/:householdId/history', requireAuth, requireRole(['manager']), async (req, res) => {
    try {
      ...
      const redFlagHistory = await storage.getHouseholdRedFlagHistory(parseInt(householdId));
      res.json(redFlagHistory);
    } catch (error) {
      console.error("Get household red flag history error:", error);
      res.status(500).json({ message: "Failed to get household red flag history" });
    }
  });

  app.post('/api/red-flags/household/:householdId/action', requireAuth, requireRole(['manager']), async (req, res) => {
    try {
      const { householdId } = req.params;
      const { actionType, actionNote } = req.body;
      const takenBy = req.session.userId!;

      if (!actionType || !actionNote) {
        return res.status(400).json({ message: "Action type and note are required" });
      }

      // Get current red flag count
      const currentRedFlagCount = await storage.getRedFlagCount(parseInt(householdId));

      // Create action record
      const action = await storage.createHouseholdAction({
        householdId: parseInt(householdId),
        actionType,
        actionNote,
        takenBy,
        redFlagCount: currentRedFlagCount,
      });

      // Get household details for notification
      const household = await storage.getHouseholdById(parseInt(householdId));
      if (!household) {
        return res.status(404).json({ message: "Household not found" });
      }

      // Send automatic notification to household generator account
      let notificationMessage = "";
      switch (actionType) {
        case "warning":
          notificationMessage = `⚠️ Warning Notice: Dear ${household.headName}, your household has been flagged for poor waste segregation. Please improve waste separation to avoid penalties. Contact your manager for guidance.`;
          break;
        case "penalty":
          notificationMessage = `🚨 Penalty Notice: Dear ${household.headName}, a penalty has been imposed on your household due to repeated poor waste segregation. Please follow proper waste management guidelines immediately.`;
          break;
        case "notice":
          notificationMessage = `📢 Important Notice: Dear ${household.headName}, ${actionNote}. Please contact your manager for more details.`;
          break;
        case "resolved":
          notificationMessage = `✅ Issue Resolved: Dear ${household.headName}, your waste management has improved! Keep up the good work with proper waste segregation.`;
          break;
        default:
          notificationMessage = `📝 Update: Dear ${household.headName}, ${actionNote}`;
      }

      // Create announcement targeted to this specific household
      ...
      res.json({
        action,
        message: "Action recorded and household notified successfully"
      });
    } catch (error) {
      console.error("Create household action error:", error);
      res.status(500).json({ message: "Failed to record household action" });
    }
  });
