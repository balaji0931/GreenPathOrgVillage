...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/tracking/status", requireAuth, requireRole(['collector']), async (req, res) => {
    try {
      ...
      // Get collector by userId  
      const collector = await storage.getCollectorByUid(userId);
      if (!collector) {
        return res.status(404).json({ message: "Collector not found" });
      }

      const activeSession = await storage.getActiveTrackingSession(collector.id);
      const hasActiveSession = !!activeSession;

      res.json({ hasActiveSession, session: activeSession });
    } catch (error) {
  ...
  app.post("/api/tracking/start", requireAuth, requireRole(['collector']), async (req, res) => {
    try {
      ...
      const existingSession = await storage.getActiveTrackingSession(collector.id);
      if (existingSession) {
        return res.status(400).json({ message: "There's already an active tracking session" });
      }

      const session = await storage.startTrackingSession(collector.id);
      res.json({ session });
    } catch (error) {
      console.error("Error starting tracking session:", error);
      res.status(500).json({ message: "Internal server error" });
  ...
  app.post("/api/tracking/end", requireAuth, requireRole(['collector']), async (req, res) => {
    try {
      ...
      const session = await storage.endTrackingSession(activeSession.id);
      res.json({ session });
    } catch (error) {
      console.error("Error ending tracking session:", error);
      res.status(500).json({ message: "Internal server error" });
  ...
  app.post("/api/tracking/location", requireAuth, requireRole(['collector']), async (req, res) => {
    try {
      ...
      const location = await storage.saveLocationUpdate(activeSession.id, latitude, longitude, accuracy);
      res.json({ location });
    } catch (error) {
      console.error("Error saving location:", error);
      res.status(500).json({ message: "Internal server error" });
  ...
  app.get("/api/manager/tracking", requireAuth, requireRole(['manager']), async (req, res) => {
    try {
      const villageId = req.session.villageId!;
      const trackingData = await storage.getTrackingDataForManager(villageId);
