...
import { CalendarIcon, Plus, Users, Home, UserCheck, Star, Trash2, Edit, UserPlus, LogOut, Settings, BarChart3, ClipboardList, UserCog, QrCode, Download, Clock, Eye, AlertCircle, MessageSquare, TrendingUp, Award, FileText, Package, AlertTriangle, Camera, Mic, CheckCircle, XCircle, Filter, Search } from "lucide-react";
...
export default function ManagerDashboard() {
  ...
  const [selectedHouseholdFilter, setSelectedHouseholdFilter] = useState<string>("");
  const { data: villageCollections = [] } = useQuery({
    queryKey: ['/api/waste-collections/village', user?.villageId, collectionsDate, selectedHouseholdFilter],
    enabled: !!user?.villageId,
  });
  ...
  const createCollectorMutation = useMutation({
    mutationFn: (data: { name: string; phone: string }) =>
      apiRequest('POST', '/api/collectors', { ...data, villageId: user?.villageId }),
    onSuccess: () => {
  ...
  const createHouseholdMutation = useMutation({
    mutationFn: (data: { headName: string; houseNumber: string; phone: string }) =>
      apiRequest('POST', '/api/households', { ...data, villageId: user?.villageId }),
    onSuccess: () => {
  ...
  const resolveComplaintMutation = useMutation({
    mutationFn: (data: { complaintId: number; managerResponse: string }) =>
      apiRequest('PUT', `/api/complaints/${data.complaintId}/resolve`, { managerResponse: data.managerResponse }),
    onSuccess: () => {
  ...
  const CreateHouseholdDialog = () => {
    ...
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button><Plus className="h-4 w-4 mr-2" />Add Household</Button>
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold">{stats?.complaintsCount || 0}</div>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button variant="outline">
              ...
              Manage Complaints ({complaints.filter(c => c.status === 'open').length})
            </Button>
          </DialogTrigger>
                    ...
                    <div className="flex justify-between items-start">
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium">{complaint.collectorName}</span>
                        ...
                        {complaint.managerResponse && (
                          <div className="mt-2 p-2 bg-muted rounded">
                            <p className="text-sm"><strong>Manager Response:</strong> {complaint.managerResponse}</p>
                          </div>
                        )}
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm p-2 bg-muted rounded">{selectedComplaint?.complaint}</p>
              </div>
              <div>
              ...
              <Button onClick={submitResolution} disabled={!managerResponse} className="w-full">
                Resolve Complaint
              </Button>
            </div>
          </DialogContent>
  ...
  const PasswordChangeDialog = () => {
    ...
    const changePasswordMutation = useMutation({
      ...
      onError: (error: any) => {
        toast({ title: "Failed to change password", description: error.message });
      },
    });
  ...
  function BulkHouseholdCreation() {
    ...
    const createHouseholdsMutation = useMutation({
      ...
      onSuccess: (response: any) => {
        ...
        queryClient.invalidateQueries({ queryKey: ['/api/households'] });
        setHouseholds([{ headName: '', houseNumber: '', phone: '', address: '' }]);
      },
      onError: (error: any) => {
        toast({ 
          title: "Error", 
    ...
    const addHousehold = () => {
      setHouseholds([...households, { headName: '', houseNumber: '', phone: '', address: '' }]);
    };
    ...
    const handleSubmit = () => {
      const validHouseholds = households.filter(h => h.headName && h.houseNumber && h.phone);
      if (validHouseholds.length === 0) {
        toast({ title: "Error", description: "Please fill required fields for at least one household", variant: "destructive" });
        return;
      }
    ...
    return (
      <div className="space-y-4">
          ...
          <Card key={index}>
            <CardContent className="p-4">
              <div className="flex justify-between items-center mb-4">
                ...
                {households.length > 1 && (
                  <Button size="sm" variant="outline" onClick={() => removeHousehold(index)}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateHousehold(index, 'headName', e.target.value)}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateHousehold(index, 'houseNumber', e.target.value)}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateHousehold(index, 'phone', e.target.value)}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateHousehold(index, 'address', e.target.value)}
        ...
        <div className="flex gap-2">
          ...
          >
            {createHouseholdsMutation.isPending ? "Creating..." : "Create Households with QR Codes"}
          </Button>
        </div>
      </div>
    );
          ...
          <>
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Button 
                  ...
                  disabled={selectedHouseholds.length === 0 || generateQRMutation.isPending}
                >
                  {generateQRMutation.isPending ? "Generating..." : `Generate QR (${selectedHouseholds.length})`}
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              {householdsWithoutQR.map(household => (
                <Card key={household.id} className="cursor-pointer hover:bg-gray-50" onClick={() => toggleHousehold(household.id)}>
                  <CardContent className="p-4">
  ...
  function QRCodeDownloadPanel({ households }: { households: Household[] }) {
    ...
    return (
      <div className="space-y-4">
        {households.length === 0 ? (
          <p className="text-center text-muted-foreground">No QR codes available for download. Generate QR codes first!</p>
        ) : (
          <>
            <div className="flex justify-between items-center">
              <p className="text-sm text-muted-foreground">
              ...
              <div className="flex gap-2">
                ...
                <Button 
                  ...
                  disabled={selectedHouseholds.length === 0 || downloadPDFMutation.isPending}
                >
                  <Download className="h-4 w-4 mr-2" />
                  {downloadPDFMutation.isPending ? "Downloading..." : `Download PDF (${selectedHouseholds.length})`}
                </Button>
              </div>
            </div>
        ...
        <div className="bg-white border-b px-3 sm:px-6 py-3 sm:py-4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-4">
            <div className="flex-1 min-w-0">
              <h1 className="text-lg sm:text-xl font-semibold truncate">Manager Dashboard</h1>
        ...
        <div className="flex flex-1 min-h-0">
          ...
          {activeTab === "overview" && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-muted-foreground">Summary of your village operations</p>
              </div>
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-4">
                                ...
                                <div className="grid grid-cols-4 gap-4 text-center">
                                  ...
                                  <div>
                                    <div className="text-lg font-bold">{stats?.averageRating?.toFixed(1) || '0.0'}</div>
                                    <div className="text-xs text-muted-foreground">Avg Rating</div>
                                  </div>
                                  <div>
                                    <div className="text-lg font-bold">{stats?.attendanceRate?.toFixed(0) || 0}%</div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {Array.isArray(allCollections) ? allCollections.filter(c => {
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">{Array.isArray(allCollections) ? allCollections.length : 0}</div>
                ...
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Avg Segregation Rating</CardTitle>
                    <Star className="h-4 w-4 text-muted-foreground" />
                  </CardHeader>
                  <CardContent>
                    <div className="text-2xl font-bold">
                    ...
                    <p className="text-xs text-muted-foreground">Segregation Quality</p>
                  </CardContent>
                </Card>
              </div>
                    ...
                    {Array.isArray(households) && households.length > 0 ? (
                      households
                        ...
                        .map(household => {
                          ...
                          const householdCollections = Array.isArray(allCollections) 
                            ? allCollections.filter(c => c.householdId === household.id)
                            : [];
                          ...
                          const filteredCollections = collectionsDateFilter
                            ? householdCollections.filter(c => {
                                const collectionDate = new Date(c.collectionDate);
                                const filterDate = new Date(collectionsDateFilter);
                                return collectionDate.toDateString() === filterDate.toDateString();
                              })
                                ...
                                <Card className="hover:shadow-md transition-shadow cursor-pointer">
                                  <CardContent className="p-3 sm:p-4">
                                    <div className="flex items-center justify-between gap-2">
                                      <div className="flex-1 min-w-0">
                                        <h3 className="font-semibold text-sm sm:text-base truncate">{household.headName}</h3>
                                        <p className="text-xs sm:text-sm text-muted-foreground truncate">
                                        ...
                                        <div className="hidden lg:grid lg:grid-cols-3 gap-4 text-center mt-2">
                                          <div>
                                            <div className="text-lg font-bold">{householdCollections.length}</div>
                                            <div className="text-xs text-muted-foreground">Collections</div>
                                          </div>
                                          <div>
                                            <div className="text-sm font-bold">
                                              {latestCollection 
                                                ? new Date(latestCollection.collectionDate).toLocaleDateString()
                                                : 'Never'
                                              }
                                            </div>
                                            <div className="text-xs text-muted-foreground">Last Collection</div>
                                          </div>
                                          <div>
                                            <div className="text-sm font-bold">
                                      ...
                                      <div className="flex flex-col items-end gap-1 flex-shrink-0">
                                        <Badge 
                                          variant={todayCollection ? "default" : latestCollection ? "secondary" : "destructive"}
                                              ...
                                              <div className="space-y-3">
                                                <div className="flex items-center justify-between">
                                                  ...
                                                  <div className="flex items-center gap-2">
                                                    ...
                                                    <Badge variant={collection.status === 'collected' ? 'default' : collection.status === 'missed' ? 'destructive' : 'secondary'}>
                                                  ...
                                                  <div className="space-y-2">
                                                    ...
                                                    <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                                                      ...
                                                      <div className="flex items-center gap-1">
                                                        ...
                                                        <span className="ml-2 text-sm font-bold">({collection.segregationRating || 0}/5)</span>
                                                      </div>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                                                      ...
                                                      <div className="flex items-center gap-1">
                                                        ...
                                                        <span className="ml-2 text-sm font-bold">({collection.plasticRating || 0}/5)</span>
                                                      </div>
                                                    </div>
                                                ...
                                                <div className="flex gap-2 pt-2 border-t">
                                                  {collection.photoUrl && (
                                                    <Button size="sm" variant="outline" onClick={() => window.open(collection.photoUrl, '_blank')}>
                                                      <Camera className="h-4 w-4 mr-1" />
                                                      View Photo
                                                    </Button>
                                                  )}
                                                  {collection.voiceUrl && (
                                                    <Button size="sm" variant="outline" onClick={() => window.open(collection.voiceUrl, '_blank')}>
                                                      <Mic className="h-4 w-4 mr-1" />
                                                      Play Voice
                                                    </Button>
                                                  )}
                                                  {!collection.photoUrl && !collection.voiceUrl && (
                                                    <p className="text-xs text-muted-foreground self-center">No media attachments</p>
                                                  )}
                                                </div>
                                              </div>
                                            </CardContent>
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-muted-foreground">View and analyze generator feedback for collectors</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                ...
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Excellent Ratings</CardTitle>
                    <Award className="h-4 w-4 text-muted-foreground" />
                  </CardHeader>
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {Array.isArray(feedbacks) ? feedbacks.filter((f: any) => (f.rating || 0) >= 4).length : 0}
                    </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {Array.isArray(feedbacks) ? feedbacks.filter((f: any) => (f.rating || 0) <= 2).length : 0}
                    </div>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Generator feedback about waste collection services</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {Array.isArray(feedbacks) && feedbacks.length > 0 ? (
                      feedbacks
                        .filter((feedback: any) => {
                          ...
                          if (feedbackDate) {
                            ...
                            if (feedbackDateObj.toDateString() !== filterDateObj.toDateString()) {
                              return false;
                            }
                          }
                          return true;
                              ...
                              <div className="flex items-start justify-between">
                                <div className="flex-1">
                                  <div className="flex items-center gap-6 mb-4">
                                    <div>
                                      <h3 className="text-lg font-semibold">{feedback.headName || 'Unknown Household'}</h3>
                                      <p className="text-sm text-muted-foreground">
                                        Household: {feedback.householdUid || 'N/A'} | House: {feedback.houseNumber || 'N/A'}
                                      </p>
                                    </div>
                                    <div>
                                      <h4 className="font-medium text-blue-600">For Collector: {feedback.collectorName || 'Unknown'}</h4>
                                      <p className="text-sm text-muted-foreground">
                                  ...
                                  <div className="grid grid-cols-2 gap-6 mb-4">
                                    <div>
                                      <span className="font-medium text-sm">Rating Given:</span>
                                      <div className="flex items-center mt-2">
                                        ...
                                        <span className="ml-3 text-lg font-medium">({feedback.rating || 0}/5)</span>
                                      </div>
                                    </div>
                                    <div>
                                      <span className="font-medium text-sm">Feedback Date:</span>
                                      <p className="text-sm mt-2">{new Date(feedback.createdAt).toLocaleDateString()}</p>
                                      <p className="text-xs text-muted-foreground">{new Date(feedback.createdAt).toLocaleTimeString()}</p>
                                    </div>
                                  </div>

                                  {feedback.remarks && (
                                    <div className="mb-4">
                                      <span className="font-medium text-sm">Generator Comments:</span>
                                  ...
                                  {feedback.collectionDate && (
                                    <div className="text-xs text-muted-foreground">
                                      Collection Date: {new Date(feedback.collectionDate).toLocaleDateString()}
                                    </div>
                                  )}
                                </div>
                                ...
                                <div className="flex flex-col items-end gap-2 ml-4">
                                  <Badge 
                                    variant={
                                      (feedback.rating || 0) >= 4 ? 'default' : 
                                      (feedback.rating || 0) >= 3 ? 'secondary' : 'destructive'
                                    }
                                    className="text-sm px-3 py-1"
                                  >
                                    {(feedback.rating || 0) >= 4 ? 'Excellent Service' : 
                                     (feedback.rating || 0) >= 3 ? 'Good Service' : 'Needs Improvement'}
                                  </Badge>
                      ...
                      <Card>
                        <CardContent className="p-8 text-center">
                          ...
                          <p className="text-lg font-medium mb-2">No Feedback Available</p>
                          <p className="text-muted-foreground">
                            No generator feedback has been recorded yet or matches your current filters.
                          </p>
                        </CardContent>
                      </Card>
                    )}
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-muted-foreground">Manage households and assign collectors</p>
                </div>
              </div>
              ...
              <div className="bg-white border rounded-lg p-1">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-1">
                  ...
                  <Button
                    variant={householdSubTab === "qr-create" ? "default" : "ghost"}
                  ...
                  <Button
                    variant={householdSubTab === "qr-download" ? "default" : "ghost"}
              ...
              {householdSubTab === "add" && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Create a new household with QR code generation</CardDescription>
                  </CardHeader>
                  <CardContent>
          ...
          {activeTab === "segregators" && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-2xl font-bold">Segregators Management</h2>
                  <p className="text-muted-foreground">Manage waste segregation workers with performance tracking</p>
                </div>
                <div className="flex gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {segregatorAttendance.filter(a => a.status === 'present').length}
                    </div>
                    <p className="text-xs text-muted-foreground">
                    ...
                    <div className="text-2xl font-bold">
                      {segregatorAttendance.length > 0 
                        ? (segregatorAttendance
                            ...
                            .reduce((sum, a) => sum + (a.workRating || 0), 0) / 
                           segregatorAttendance.filter(a => a.workRating).length).toFixed(1)
                        : '0.0'
                      }
                    </div>
              ...
              {segregatorsLoading ? (
                ...
                <div className="space-y-4">
                  {segregators.map(segregator => {
                    const todayAttendance = segregatorAttendance.find(a => a.segregatorId === segregator.id);
                    const detailedRecord = detailedAttendance.find(a => a.segregatorId === segregator.id);
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-4">
                                <div>
                                  <h3 className="font-semibold">{segregator.name}</h3>
                                  <p className="text-sm text-muted-foreground">
                                ...
                                {detailedRecord && (
                                  <div className="grid grid-cols-3 gap-4 text-center">
                                    <div>
                                      <div className="text-lg font-bold">{detailedRecord.workHours || 0}h</div>
                                      <div className="text-xs text-muted-foreground">Work Hours</div>
                                    </div>
                                    <div>
                                      <div className="text-lg font-bold">{detailedRecord.workRating || 0}/5</div>
                              ...
                              {(todayAttendance || detailedRecord) && (
                                <div className="mt-3 p-2 bg-muted rounded-lg">
                                  <div className="flex items-center justify-between text-sm">
                                    ...
                                    <div className="flex items-center gap-2">
                                      <Badge variant={
                                        (todayAttendance?.status === 'present' || detailedRecord?.status === 'present') ? "default" :
                                        (todayAttendance?.status === 'half_day' || detailedRecord?.status === 'half_day') ? "secondary" : "destructive"
                                      }>
                                        {(todayAttendance?.status || detailedRecord?.status || 'absent').replace('_', ' ').toUpperCase()}
                                      </Badge>
                                      {(todayAttendance?.workRating || detailedRecord?.workRating) && (
                                        <span className="text-muted-foreground">
                                          Rating: {todayAttendance?.workRating || detailedRecord?.workRating}/5
                                        </span>
                                      )}
                                    </div>
                                  </div>
                                  {(todayAttendance?.remarks || detailedRecord?.remarks) && (
                                    <div className="mt-1 text-xs text-muted-foreground">
                                      <strong>Remarks:</strong> {todayAttendance?.remarks || detailedRecord?.remarks}
                                    </div>
                                  )}
                                </div>
                              )}
                            ...
                            <div className="flex items-center gap-2">
                              <Badge variant={segregator.isActive ? "default" : "secondary"}>
                  ...
                  {segregators.length === 0 && (
                    <Card>
                      <CardContent className="p-8 text-center">
                        <p className="text-muted-foreground">No segregators found. Add your first segregator!</p>
                      </CardContent>
                    </Card>
                  )}
                </div>
          ...
          {activeTab === "attendance" && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-muted-foreground">Mark and track segregator attendance</p>
              </div>
              ...
              <Card>
                <CardContent className="p-6">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      {segregators.map(segregator => {
                        const attendance = segregatorAttendance.find(a => a.segregatorId === segregator.id);

                        return (
                          <div key={segregator.id} className="flex items-center justify-between p-4 border rounded-lg">
                            <div>
                              <h3 className="font-semibold">{segregator.name}</h3>
                              <p className="text-sm text-muted-foreground">Phone: {segregator.phone}</p>
                              {attendance && (
                                <div className="mt-2 space-y-1">
                                  <Badge variant={
                                    attendance.status === 'present' ? 'default' :
                                    attendance.status === 'half_day' ? 'secondary' : 'destructive'
                                  }>
                            ...
                            <AttendanceMarkingDialog segregator={segregator} />
                          </div>
                        );
                      })}
                      {segregators.length === 0 && (
                        <p className="text-center text-muted-foreground">No segregators found. Add segregators first!</p>
                      )}
                    </div>
                  </div>
                </CardContent>
          ...
          {activeTab === "issues" && (
            <div className="space-y-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {Array.isArray(allIssues) && allIssues.length > 0 ? allIssues
                      .filter(issue => {
                                ...
                                <div className="flex-1">
                                  ...
                                  {issue.managerReply && (
                                    <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                                      <p className="text-sm"><strong>Manager Reply:</strong> {issue.managerReply}</p>
                                    </div>
                                  )}
                                </div>
                                <div className="flex gap-2 ml-4">
                                  {issue.photoUrl && (
                                    <Button size="sm" variant="outline" onClick={() => window.open(issue.photoUrl, '_blank')}>
                                      <Eye className="h-4 w-4 mr-1" />
                                      Photo
                                    </Button>
                                  )}
          ...
          {selectedIssue && (
            <form onSubmit={(e) => {
              ...
              <div>
                ...
                <Textarea id="description" value={selectedIssue.description} disabled rows={3} />
              </div>
              <div>
              ...
              <Button type="submit" disabled={updateIssueMutation.isPending} className="w-full">
