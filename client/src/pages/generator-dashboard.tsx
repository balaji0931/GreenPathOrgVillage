...
export default function GeneratorDashboard() {
  ...
  const createFeedbackMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/waste-collections/household"] });
  ...
  const monthlyStats = {
    ...
    avgSegregationRating: collectionHistory?.length 
      ? (collectionHistory.reduce((sum: number, item: any) => sum + (item.segregationRating || 0), 0) / collectionHistory.length).toFixed(1)
      : "0.0",
    thisMonthCollections: collectionHistory?.filter((item: any) => {
      ...
      <div className="bg-green-600 text-white p-4 sticky top-0 z-10">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-sm font-medium">{user?.name?.split(' - ')[1] || user?.name}</p>
                    ...
                    {collectionHistory.slice(0, 3).map((collection: any) => (
                      <div key={collection.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center space-x-3">
                          <div className={`w-3 h-3 rounded-full ${collection.status === 'collected' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                          <div>
                            <p className="text-sm font-medium">{new Date(collection.collectionDate).toLocaleDateString()}</p>
                            <p className="text-xs text-gray-600">
                              Rating: {collection.segregationRating ? `${collection.segregationRating}/5` : 'N/A'}
                            </p>
                          </div>
                        </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div>
                    ...
                    <Progress value={(monthlyStats.thisMonthCollections / 30) * 100} className="h-2" />
                  </div>
                  ...
                  <div>
                    ...
                    <Progress value={(parseFloat(monthlyStats.avgSegregationRating) / 5) * 100} className="h-2" />
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900">{monthlyStats.totalCollections}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Average Rating</p>
                    <p className="text-2xl font-bold text-gray-900">{monthlyStats.avgSegregationRating}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <div className="space-y-3">
                {collectionHistory
                  .filter((collection: any) => {
                    ...
                    const collectionDate = new Date(collection.collectionDate).toLocaleDateString().toLowerCase();
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          <div className={`w-3 h-3 rounded-full ${collection.status === 'collected' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                          <p className="font-medium text-sm">{new Date(collection.collectionDate).toLocaleDateString()}</p>
                          <Badge variant={collection.status === 'collected' ? 'default' : 'destructive'} className="text-xs">
                            {collection.status}
                          </Badge>
                        </div>
                        
                        <div className="space-y-1 text-xs text-gray-600">
                          <p>Segregation: {collection.segregationRating ? `${collection.segregationRating}/5 ⭐` : 'Not rated'}</p>
                          <p>Cleanliness: {collection.plasticRating ? `${collection.plasticRating}/5 ⭐` : 'Not rated'}</p>
                          {collection.observations && collection.observations.length > 0 && (
                            <p>Notes: {collection.observations.join(', ')}</p>
                          )}
                          {collection.remarks && (
                ...
                {/* View All Button */}
                {!showAllCollections && collectionHistory && collectionHistory.length > 10 && (
              ...
              <div className="space-y-3">
                {issues
                  .filter((issue: any) => {
                    ...
                    if (issueFilter === 'In Progress') return issue.status === 'in_progress';
      ...
      <Dialog open={showFeedbackModal} onOpenChange={setShowFeedbackModal}>
        <DialogContent className="w-[96vw] max-w-lg max-h-[90vh] overflow-y-auto p-4 mx-auto">
          ...
          <div className="space-y-5">
            ...
            {selectedCollection && (
              <div className="p-3 bg-gray-50 rounded-lg">
                <p className="text-sm font-medium text-gray-800">
                  Collection Date: {new Date(selectedCollection.collectionDate).toLocaleDateString()}
                </p>
                <p className="text-xs text-gray-600">
                  Status: {selectedCollection.status} | 
                  Segregation Rating: {selectedCollection.segregationRating || 'N/A'}/5
                </p>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-center space-x-2 mt-2 p-4 bg-gray-50 rounded-lg">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    key={star}
                    onClick={() => setCollectorFeedback({ ...collectorFeedback, rating: star })}
            ...
            {/* Comments */}
            <div className="space-y-2">
              <Label htmlFor="remarks" className="text-sm font-semibold text-gray-800 flex items-center">
                💬 Additional Comments (Optional)
              </Label>
              <Textarea
                id="remarks"
                value={collectorFeedback.remarks}
                onChange={(e) => setCollectorFeedback({ ...collectorFeedback, remarks: e.target.value })}
                ...
                maxLength={300}
              />
              <p className="text-xs text-gray-500">{collectorFeedback.remarks.length}/300 characters</p>
            </div>
            ...
            <div className="flex flex-col space-y-3 pt-4 border-t">
              <Button 
                ...
                disabled={createFeedbackMutation.isPending || collectorFeedback.rating === 0}
