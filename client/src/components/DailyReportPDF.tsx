...
const sec = (t: string) => `
  <div style="display:flex;align-items:center;gap:12px;margin:32px 0 16px;">
    <span style="background:${C.brandLight};color:${C.brandDark};padding:6px 14px;border-radius:6px;font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:0.18em;display:inline-block;">${t}</span>
    <div style="flex:1;height:1.5px;background:${C.brandLight};"></div>
  </div>`;

const th = (t: string, a = "left") =>
  `<th style="padding:10px 14px;font-size:10px;font-weight:800;color:#fff;text-align:${a};text-transform:uppercase;letter-spacing:0.1em;white-space:nowrap;">${t}</th>`;

const td = (t: string, o: { a?: string; b?: boolean; c?: string; m?: boolean } = {}) =>
  `<td style="padding:10px 14px;font-size:12px;text-align:${o.a || "left"};font-weight:${o.b ? "800" : "600"};color:${o.c || C.s700};${o.m ? "font-family:'Courier New',monospace;" : ""}">${t}</td>`;
...
const noData = (title: string, message: string) => `
  <div style="background:${C.s50};border:2px dashed ${C.s200};border-radius:12px;padding:24px 20px;text-align:center;margin-top:8px;">
    <div style="font-size:11px;font-weight:800;color:${C.s500};margin-bottom:6px;">${title}</div>
    <div style="font-size:10px;font-weight:600;color:${C.s400};">${message}</div>
  </div>`;
...
function buildFullReport(d: PDFReportData): string {
  ...
  const kpi = (label: string, value: string, sub: string, color: string) => `
    <div style="background:${C.s50};border:1.5px solid ${C.s200};border-radius:12px;padding:16px 20px;border-left:4px solid ${color};display:flex;align-items:center;justify-content:space-between;gap:14px;">
      <div style="flex:1;">
        <div style="font-size:11px;font-weight:800;color:${C.s900};margin-bottom:4px;">${label}</div>
        <div style="font-size:10px;color:${C.s400};font-weight:600;">${sub}</div>
      </div>
  ...
  const pulseHead = d.pulses.map(p =>
    `<th style="padding:10px 6px;font-size:10px;font-weight:800;color:${C.s500};text-align:center;text-transform:uppercase;letter-spacing:0.08em;border-bottom:2px solid ${C.s200};">${p.day}</th>`
  ).join("");
  const pulseBars = d.pulses.map(p => {
    const h = Math.max(p.collections / maxC * 50, 5);
    return `<td style="padding:10px 6px;text-align:center;vertical-align:bottom;">
      <div style="display:flex;flex-direction:column;align-items:center;gap:5px;">
        <span style="font-size:12px;font-weight:900;color:${C.s900};">${p.collections}</span>
        <div style="width:28px;height:${h}px;background:linear-gradient(180deg,${C.brand},${C.brandDark});border-radius:4px;"></div>
        <span style="font-size:9px;font-weight:700;color:${C.amber};">${p.rating}/5</span>
      </div>
    </td>`;
  }).join("");
  ...
  const matRows = matItems.map((m, i) => {
    ...
    return `<tr style="background:${i % 2 === 0 ? C.white : C.s50};">
      ${td(`<span style="display:inline-flex;align-items:center;gap:7px;"><span style="width:9px;height:9px;border-radius:3px;background:${m.color};display:inline-block;"></span>${m.label}</span>`, { b: true })}
      ${td(`${m.kg.toFixed(1)} kg`, { a: "right", b: true, m: true })}
      ${td(`${pct.toFixed(1)}%`, { a: "right", c: C.s500 })}
    </tr>`;
  }).join("");
  ...
  const vCards = activeV.map(v => `
    <div class="avoid-break" style="background:${C.s50};border:1.5px solid ${C.s200};border-radius:10px;padding:14px 16px;margin-bottom:10px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid ${C.s200};">
        <div>
          <span style="font-size:12px;font-weight:900;color:${C.s900};">${v.vehicleName}</span>
          <span style="font-size:9px;color:${C.s400};font-weight:600;margin-left:8px;font-family:'Courier New',monospace;background:${C.s100};padding:2px 6px;border-radius:4px;">${v.registrationNumber}</span>
        </div>
        <div style="background:${C.brand};color:#fff;padding:4px 12px;border-radius:8px;font-size:14px;font-weight:900;">${v.count}</div>
      </div>
      <div style="display:flex;gap:20px;flex-wrap:wrap;">
        <div><div style="font-size:7px;font-weight:800;color:${C.s400};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:2px;">Collectors</div><div style="font-size:10px;font-weight:700;color:${C.s700};">${v.collectorNames || "--"}</div></div>
        <div><div style="font-size:7px;font-weight:800;color:${C.s400};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:2px;">Start</div><div style="font-size:10px;font-weight:700;color:${C.s700};">${fmtTime(v.startTime)}</div></div>
        <div><div style="font-size:7px;font-weight:800;color:${C.s400};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:2px;">End</div><div style="font-size:10px;font-weight:700;color:${C.s700};">${fmtTime(v.endTime)}</div></div>
        <div><div style="font-size:7px;font-weight:800;color:${C.s400};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:2px;">Work Time</div><div style="font-size:10px;font-weight:800;color:${C.green};">${fmtMs(v.totalWorkMs)}</div></div>
        <div><div style="font-size:7px;font-weight:800;color:${C.s400};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:2px;">Break Time</div><div style="font-size:10px;font-weight:800;color:${C.amber};">${fmtMs(v.totalBreakMs)}</div></div>
      </div>
    </div>
  `).join("");
  ...
  const hourHeaders = hours.map(hr =>
    `<th style="padding:5px 4px;font-size:7px;font-weight:800;color:#fff;text-align:center;text-transform:uppercase;letter-spacing:0.04em;white-space:nowrap;">${hr}</th>`
  ).join("");
  const vehicleHourRows = vehicles.map((v, vi) => {
    const cells = hourly.map(h => {
      const val = h[v.name] || 0;
      return `<td style="padding:5px 4px;font-size:9px;text-align:center;font-weight:${val > 0 ? '800' : '500'};color:${val > 0 ? C.s900 : C.s400};">${val || "-"}</td>`;
    }).join("");
    const rowTotal = hourly.reduce((s, h) => s + (h[v.name] || 0), 0);
    return `<tr style="background:${vi % 2 === 0 ? C.white : C.s50};">
      <td style="padding:5px 8px;font-size:9px;font-weight:800;color:${C.s700};white-space:nowrap;">${v.name}</td>
      ${cells}
      <td style="padding:5px 6px;font-size:9px;text-align:center;font-weight:900;color:${C.brand};">${rowTotal}</td>
    </tr>`;
  }).join("");
  const totalHourlyCells = hourly.map(h => {
    const t = vehicles.reduce((s, v) => s + (h[v.name] || 0), 0);
    return `<td style="padding:5px 4px;font-size:9px;text-align:center;font-weight:900;color:${C.totTx};">${t || "-"}</td>`;
  }).join("");
  ...
  return `
    <div style="width:${PW}px;padding:${M}px;background:#fff;font-family:'Segoe UI',Roboto,-apple-system,sans-serif;color:${C.s900};box-sizing:border-box;">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:20px 0 20px;margin-bottom:20px;border-bottom:2.5px solid ${C.brand};">
        <div style="flex:0 0 auto;">${logoImg(44)}</div>
        <div style="flex:1;text-align:center;">
          <div style="font-size:11px;font-weight:800;color:${C.s500};text-transform:uppercase;letter-spacing:0.25em;">Daily Operations Report</div>
        </div>
        <div style="flex:0 0 auto;text-align:right;">
          <div style="font-size:15px;font-weight:900;color:${C.s900};margin-bottom:2px;">${d.villageName}</div>
          <div style="font-size:11px;color:${C.s500};font-weight:600;">${fmtDate(d.date)}</div>
        </div>
      </div>

      <!-- META -->
      <div style="display:flex;justify-content:center;gap:1px;margin-bottom:24px;background:${C.s200};border-radius:10px;overflow:hidden;">
        <div style="flex:1;background:${C.s50};padding:12px 14px;text-align:center;">
          <div style="font-size:8px;font-weight:800;color:${C.s400};text-transform:uppercase;letter-spacing:0.14em;margin-bottom:4px;">Generated By</div>
          <div style="font-size:12px;font-weight:800;color:${C.s900};">${d.managerName}</div>
        </div>
        <div style="flex:1;background:${C.s50};padding:12px 14px;text-align:center;">
          <div style="font-size:8px;font-weight:800;color:${C.s400};text-transform:uppercase;letter-spacing:0.14em;margin-bottom:4px;">Report ID</div>
          <div style="font-size:12px;font-weight:800;color:${C.s900};font-family:'Courier New',monospace;">${genId(d.villageId, d.date)}</div>
        </div>
        <div style="flex:1;background:${C.s50};padding:12px 14px;text-align:center;">
          <div style="font-size:8px;font-weight:800;color:${C.s400};text-transform:uppercase;letter-spacing:0.14em;margin-bottom:4px;">Generated At</div>
          <div style="font-size:12px;font-weight:800;color:${C.s900};">${new Date().toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", hour12: true, timeZone: "Asia/Kolkata" })} IST</div>
        </div>
      </div>
      ...
      <div class="avoid-break">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
          ...
          ${d.materialData.isLogged ? kpi("Waste Diversion", `${Math.round(divPct)}%`, `${diverted.toFixed(1)}kg diverted`, statusColor(divPct, 70, 40)) : kpi("Waste Diversion", "--", "No material log today", C.s400)}
        </div>
      </div>
      ...
      <div class="avoid-break">
        ...
        <div style="background:${C.s50};border:1.5px solid ${C.s200};border-radius:12px;padding:18px 16px 14px;overflow:hidden;">
      ...
      ${d.materialData.isLogged ? `
        <div class="avoid-break">
          ${sec("Waste Material Breakdown")}
          <table style="width:100%;border-collapse:collapse;border:1.5px solid ${C.s200};border-radius:10px;overflow:hidden;margin-bottom:14px;">
          ...
          <!-- DIVERSION CARD -->
          <div style="background:linear-gradient(135deg,${C.s50},${C.brandLight});border:1.5px solid ${C.s200};border-radius:12px;padding:16px 20px;display:flex;align-items:center;gap:20px;">
            <div style="flex:1;">
              <div style="font-size:8.5px;font-weight:800;color:${C.s500};text-transform:uppercase;letter-spacing:0.12em;margin-bottom:8px;">Waste Diversion Rate</div>
              <div style="height:10px;background:${C.s200};border-radius:5px;overflow:hidden;margin-bottom:8px;">
                <div style="height:100%;width:${Math.min(divPct, 100)}%;background:linear-gradient(90deg,${divColor},${divColor}dd);border-radius:5px;"></div>
              </div>
              <div style="display:flex;justify-content:space-between;font-size:8.5px;font-weight:700;">
                <span style="color:${C.green};">Diverted: ${diverted.toFixed(1)} kg</span>
                <span style="color:${C.red};">Landfill: ${(totalWaste - diverted).toFixed(1)} kg</span>
              </div>
            </div>
            <div style="text-align:center;min-width:80px;padding:10px;background:${C.white};border-radius:10px;border:1.5px solid ${divColor}40;">
              <div style="font-size:28px;font-weight:900;color:${divColor};line-height:1;">${Math.round(divPct)}%</div>
              <div style="font-size:7.5px;font-weight:800;color:${divColor};text-transform:uppercase;letter-spacing:0.1em;margin-top:3px;">${divLabel}</div>
            </div>
          </div>
        </div>
      ` : `
        <div class="avoid-break">
          ${sec("Waste Material Breakdown")}
          ${noData("No Material Data", "Waste type logs were not recorded for this date. Material breakdown and diversion rate will appear once collectors log waste types.")}
        </div>
      `}
      ...
      <div class="avoid-break">
        ${grandTotal > 0 ? `
          ...
          <table style="width:100%;border-collapse:collapse;border:1.5px solid ${C.s200};border-radius:10px;overflow:hidden;">
            <thead><tr style="background:linear-gradient(135deg,${C.brandDark},${C.brand});">
              <th style="padding:5px 8px;font-size:7px;font-weight:800;color:#fff;text-align:left;text-transform:uppercase;letter-spacing:0.06em;">Vehicle</th>
              ${hourHeaders}
              <th style="padding:5px 6px;font-size:7px;font-weight:800;color:#fff;text-align:center;text-transform:uppercase;letter-spacing:0.06em;">Total</th>
            </tr></thead>
            <tbody>
              ${vehicleHourRows}
              <tr style="background:${C.totBg};border-top:2px solid ${C.amber};">
                <td style="padding:5px 8px;font-size:9px;font-weight:900;color:${C.totTx};">TOTAL</td>
                ${totalHourlyCells}
                <td style="padding:5px 6px;font-size:9px;text-align:center;font-weight:900;color:${C.totTx};">${grandTotal}</td>
              </tr>
            </tbody>
          </table>
        ` : `
      ...
      <div style="text-align:center;padding-top:24px;margin-top:40px;padding-bottom:20px;border-top:2.5px solid ${C.brand};">
        <div style="font-size:11px;color:${C.s500};font-weight:600;line-height:1.8;">
          ...
          <span style="font-size:9px;color:${C.s400};">${new Date().getFullYear()} GreenPath. All rights reserved.</span>
        </div>
      </div>
    </div>
  `;
...
export async function generateDailyReportPDF(
  ...
  try {
    ...
    const a4W = 210;
    const a4H = 297;
    const marginMm = 14; // header/footer reserved space
    ...
    // Capture canvas at scale 2 for high-res
    const SCALE = 2;
